...
public class RequestProjectCommits {
	public static Logger LOG = Logger.getLogger(RequestProjectCommits.class);
...
//		request(reposList);
	...
	public static void request() {
		String savePath = "E:\\gitinsight\\data\\commits\\";
...
//				}
				...
				String [] reInfo = HtmlUtil.requestPageByGetReLink("https://api.github.com/repos/" + repo + "/commits?page=1&per_page=100&access_token=" + token,//getToken(tokens), 
						savePath + saveFilePath + "\\" + saveFilePath + "commits_1" + ".json");
				...
				if(checkFile.list().length == 1) {
					...
					LOG.error("connect 3 tims still fail. The url is:" + "https://api.github.com/repos/" + repo + "/commits?page=1&per_page=100&access_token=" + token);//getToken(tokens));
				}
				

				for(int i=2; i<=lastPage; i++){
					Thread.sleep(800);
					HtmlUtil.requestPageByGet("https://api.github.com/repos/" + repo + "/commits?page=" + i +"&per_page=100&access_token=" + token,//getToken(tokens), 
							savePath + saveFilePath + "\\" + saveFilePath + "commits_" + (fileNo++) + ".json");
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
